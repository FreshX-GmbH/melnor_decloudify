...
function sendMessage (clients, event, data, channel = settings.mac) {
    clients.forEach(function each(wsClient) {
        ...
        wslog.debug(JSON.stringify({"event":event, "data": "\""+data+"\"", "channel": channel.toLowerCase()}));
        wsClient.send(JSON.stringify({"event":event, "data": "\""+data+"\"", "channel": channel.toLowerCase()}));
    });
}
...
function sendLongMessage (clients, event, data, channel = settings.mac){
    clients.forEach(function each(wsClient) {
  	...
  	wslog.complete(`Sent buffer ${buffer.toString('hex').replace(/(.{4})/g,"$1:").replace(/:$/, '')}`);
        wsClient.send(JSON.stringify({"event":event, "data": buffer.toString('base64'), "channel": channel.toLowerCase()}));
    });
}
